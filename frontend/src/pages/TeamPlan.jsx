...
const TeamPlan = ({ auth }) => {
  ...
  const getResetTime = (planType) => {
    ...
    switch (planType.toLowerCase()) {
      ...
      case "premium":
        ...
        break;
      default:
        nextReset.setDate(now.getDate() + 7); // Default to 7 days for Team
    }
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      ...
      try {
        ...
        // Filter for Team type
        const teamPlans = metadata.filter((plan) => plan.type === "Team");

        if (teamPlans.length === 0) {
          throw new Error("No team plans available. Please contact support.");
        }

        const planData = teamPlans.map((plan) => {
          ...
          if (!subscriptionId) {
            console.error("TeamPlan - Missing subscriptionId for plan:", plan);
          ...
          return {
            ...
            tag: plan.tag || "", // Use tag for display (e.g., "Learn Together")
          };
        });
        ...
        console.error("TeamPlan - API Error:", {
          message: err.message,
  ...
  const handleSubscribe = (plan) => {
    ...
    if (!userId) {
      navigate("/login", { state: { from: "/subscription/team" } });
      return;
    }
  ...
  const getButtonText = (type) => {
    switch (type.toLowerCase()) {
      case "basic":
        return "Get Basic Team Plan";
      case "pro":
        return "Get Pro Team Plan";
      case "premium":
        return "Get Premium Team Plan";
      default:
  ...
  return (
    <div className="subscription-plans-container">
      <div className="subscription-plans">
        <h2 className="plans-main-title">Empower Your Team with Premium Learning</h2>
        {error && (
        ...
        <div className="subscription-plans-category">
          {plans.length === 0 && !error ? (
            <div className="no-plans-message">No team plans available at the moment.</div>
          ) : (
            plans.map((plan) => {
              const { days, hours, minutes, seconds } = formatTime(timeLeft[plan.name] || 0);
              const tagClass = plan.tag ? plan.tag.toLowerCase().replace(/\s+/g, "-") : ""; // e.g., "Learn Together" -> "learn-together"
