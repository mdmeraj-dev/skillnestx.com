...
const VerifyEmail = ({ onAuthSuccess = () => {} }) => {
  ...
  const maskEmail = (email) => {
    ...
    const maskedPart = local.length > 8 ? "*****" : "*".repeat(local.length - 3);
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      console.debug("Verifying OTP:", { email, otp: fullOtp, traceId });
      ...
      console.debug("Verify OTP response:", { data, traceId });

      if (!response.ok) {
        const errorMessage = getErrorMessage(data, "Verification failed. Please try again.");
        throw new Error(errorMessage);
      }

      if (data.emailStatus && !data.emailStatus.success) {
        setMessage(
          `${data.message}. Note: ${data.emailStatus.message}. Check your spam folder or contact support.`
        );
      } else {
      ...
      setTimeout(() => {
        navigate(data.redirectUrl || "/", { state: { fromVerifyEmail: true } });
      }, 1500);
    } catch (err) {
      console.error("VerifyEmail error:", { message: err.message, traceId });
      ...
      if (
        ...
        setTimeout(() => navigate(err.message.includes("log in") ? "/login" : "/signup"), 2000);
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleResend = async () => {
    if (resendLoading || resendCooldown > 0 || resendCount >= MAX_RESEND_ATTEMPTS) return;
    ...
    try {
      ...
      console.debug("Resending OTP:", { email, traceId });
      ...
      const data = await response.json();
      console.debug("Resend OTP response:", { data, traceId });

      if (!response.ok) {
        const errorMessage = getErrorMessage(data, "Failed to resend OTP. Please try again.");
        throw new Error(errorMessage);
      }
      ...
      console.error("Resend OTP error:", { message: err.message, traceId });
      setMessage(err.message);
      setMessageType("error");
      setOtp(["", "", "", "", "", ""]);
      ...
      <div className="verify-email-modal-container">
        ...
        <div className="verify-email-instructions">
          <p className="verify-email-instruction-text">
            ...
            <span className="highlight-email">{maskedEmail}</span>
          </p>
          <p className="verify-email-instruction-text">
        ...
        <div className="verify-email-support-note">
          <p>
            ...
            <a href="mailto:support@skillnestx.com">contact support</a> if you
            need help.
          </p>
        </div>
      </div>
