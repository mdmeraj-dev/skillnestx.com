...
const Signup = ({ onClose, onAuthSuccess, onSignupSuccess }) => {
          ...
          if (error.response?.status === 401 && error.response.data?.needsRefresh) {
            try {
              ...
              if (refreshResponse.data.success) {
                ...
                localStorage.removeItem("keepLoggedIn");
                const retryResponse = await axios.get(`${BASE_URL}/api/user/current`, {
                  headers: {
                ...
                if (retryResponse.data.success) {
                  if (typeof onAuthSuccess === "function") {
                    onAuthSuccess(retryResponse.data.user);
                  }
                  if (typeof onSignupSuccess === "function") {
                    onSignupSuccess();
                  }
              ...
              logger.error("Refresh token error", { error: refreshError.message, traceId });
            }
          }
          logger.error("Error fetching user data", { error: error.message, traceId });
          setError(
            error.response?.data?.code === "INVALID_SESSION_TOKEN"
              ? "Invalid session. Please try signing up again."
  ...
  }, [location, navigate, onAuthSuccess, onSignupSuccess, onClose, BASE_URL]);
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        } else if (data.code === "INVALID_SESSION_TOKEN") {
          setError("Invalid session. Please try signing up again.");
        } else {
          setError(data.message || "Something went wrong. Please try again.");
        }
        logger.error("Signup error", { error: error.message, traceId, code: data?.code });
      } else {
        logger.error("Signup error", { error: error.message, traceId });
  ...
  const handleLinkNavigation = (e, path) => {
    ...
    // Use setTimeout to ensure modal closure completes before navigation
    setTimeout(() => {
